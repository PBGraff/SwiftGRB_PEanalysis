...
def plotRzPost(datafile, N, maxlike, zdatafile, outfile, annotate = False):
    ...
    factor = 4.0 * np.pi / 6.0 * 0.8 * dV * df / (1.0 + z)
    fig, ax = plt.subplots(2,1, figsize=(6,8))
    fig.subplots_adjust(hspace=0.05)
    hist_n, hist_bin, hist_patch = ax[1].hist(zdata, bins=Nbins, range=[0,10], color='red', alpha=0.5,
        label=r'$N_{\mathrm{obs}}$')
    dz = 10.0 / Nbins
    ax[1].cla()
    for i in range(N):
        n = data[samples[i],:]
        ...
        ax[1].plot(z, Rz(z,n) * factor, '-b', lw=1, alpha=0.1)
    ax[0].plot(z, Rz(z,maxlike), '-k', lw=2, label='max likelihood')
    ax[1].plot(z, Rz(z,maxlike) * factor, '-k', lw=2, label=r'$N_{\mathrm{exp}}$')
...
for model in ['NN','RF','AB']:
    ...
    figure = triangle.corner(data, labels=[r'$n_0$',r'$n_1$',r'$n_2$', r'$z_1$', r'$N_{\rm exp}$'],
                             ...
                             range=[(0,1.6),(0.7,4.0),(-6,0),(1.8,10),(2500,12000)],
                             maxlike=maxlike[model], label_kwargs={"fontsize": 20})
